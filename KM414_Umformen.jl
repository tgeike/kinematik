...
begin 
	...
	L = 40u"cm"
	lH = 2*L-l
	sSt = sqrt(l^2 - (L-0.5*lH)^2)
	θSt = asin((L - 0.5*lH)/l)
end;

# ╔═╡ bc27c86a-83de-44b6-a47f-0a679b070e8c
...
und 
```math
s_1(\theta)= L-l\sin\theta\;.
```
"""

# ╔═╡ 18812336-f1f3-4318-aba7-e945e6d32f59
s2(s1) = sqrt(l^2 - (L-s1)^2) - sSt;

# ╔═╡ 6be90123-5cb6-41a6-86bd-6931db15d739
s1(θ) = L - l*sin(θ);

# ╔═╡ 40052345-c4fc-4422-a370-a4deb829258a
...
# ╔═╡ 8eca139e-a03f-43aa-8cd5-837e53fdd335
 with_theme(theme_latexfonts()) do
	s1_tab = LinRange(0.5*lH,L,100)
	θ_tab = LinRange(0,θSt,100)
	f = Figure(size=(650,450),fontsize=20)
	ax1 = Axis(f[1,1],xlabel=L"$s_1-l_\mathrm{H}/2$",ylabel=L"$s_2$",title="") 
	lines!(ax1,(s1_tab.-0.5*lH),s2.(s1_tab))
	ax2 = Axis(f[2,1],xlabel=L"$\theta\; [°]$",ylabel=L"$s_2$",title="") 
	lines!(ax2,rad2deg.(θ_tab),s2.(s1.(θ_tab)))
	f
end
...
# ╔═╡ 40a967c4-47e8-4cba-a64a-35df1e255c89
 with_theme(theme_latexfonts()) do
	...
	θ_tab = LinRange(0,θSt,100)
	f = Figure(size=(650,450),fontsize=20)
	ax1 = Axis(f[1,1],xlabel=L"$s_1-l_\mathrm{H}/2$ [cm]",ylabel=L"$s_2$ [cm]",title="") 
	lines!(ax1,ustrip.(u"cm",(s1_tab.-0.5*lH)),ustrip.(u"cm",s2.(s1_tab)))
	ax2 = Axis(f[2,1],xlabel=L"$\theta\; [°]$",ylabel=L"$s_2$ [cm]",title="") 
	lines!(ax2,rad2deg.(θ_tab),ustrip.(u"cm",s2.(s1.(θ_tab))))
	f

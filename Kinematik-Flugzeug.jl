...
# ╔═╡ 29330db2-abe0-43a3-8c22-783f6fe4583c
using DifferentialEquations,CairoMakie,Unitful,LaTeXStrings

# ╔═╡ eb07324c-24fb-11ef-18c1-e512a2d3bece
md"""
# Technische Berechnungen mit Julia 
## Bewegung eines Flugzeugs
"""

# ╔═╡ bf985c39-7304-4f87-9295-d1eeba4a0f9a
vW = 30.0u"km/hr"

# ╔═╡ fc33d3ea-d07b-4a78-9d1a-3a37940a671f
vE = 200.0u"km/hr"

# ╔═╡ d0a53e3d-5984-4a99-a616-440bd4c3793e
struct Systemparameter 
	vW::Float64
	vE::Float64
end

# ╔═╡ 7b5e23f6-4d3e-4a0f-8b46-48bef1173644
param1 = Systemparameter(ustrip(u"m/s",vW),ustrip(u"m/s",vE))

# ╔═╡ fa908125-51ae-45ca-9e22-d57fd89504fa
param2 = Systemparameter(ustrip(u"m/s",5*vW),ustrip(u"m/s",vE))

# ╔═╡ 8ab4d178-89fa-40aa-a07f-117f85a10a22
function dgl!(F,z,p,t)
	d = sqrt(z[1]^2 + z[2]^2)
	F[1] = -p.vE*z[1]/d
	F[2] = -p.vE*z[2]/d + p.vW
end
...
function bahnkurve(r0,p;tE=1000.0) 
	tspan = (0.0,tE)
	z_start = r0
...
begin 
	sol1a = bahnkurve([2e4;-4e3],param1)
	sol1b = bahnkurve([2e4;5e3],param1)
	sol2a = bahnkurve([2e4;-4e3],param2) 
	sol2b = bahnkurve([2e4;5e3],param2)
end;

# ╔═╡ 9cbd1c7b-1aa5-4a7e-9052-1eea915d8f2a
with_theme(theme_latexfonts()) do 
	f = Figure(size=(650,400),fontsize=20)
	ax1 = Axis(f[1,1],xlabel=L"$x$ [km]",ylabel=L"$y$ [km]",aspect=2.0,title="Schwacher Wind")
	lines!(ax1,sol1a[1,:]/1e3,sol1a[2,:]/1e3)
	xlims!(ax1,-5,25)
	ylims!(ax1,-5,10)
	ax2 = Axis(f[2,1],xlabel=L"$x$ [km]",ylabel=L"$y$ [km]",aspect=2.0)
	lines!(ax2,sol1b[1,:]/1e3,sol1b[2,:]/1e3)
	xlims!(ax2,-5,25)
	ylims!(ax2,-5,10)
	ax3 = Axis(f[1,2],xlabel=L"$x$ [km]",ylabel=L"$y$ [km]",aspect=2.0,title="Starker Wind")
	lines!(ax3,sol2a[1,:]/1e3,sol2a[2,:]/1e3)
	xlims!(ax3,-5,25)
	ylims!(ax3,-5,10)
	ax4 = Axis(f[2,2],xlabel=L"$x$ [km]",ylabel=L"$y$ [km]",aspect=2.0)
	lines!(ax4,sol2b[1,:]/1e3,sol2b[2,:]/1e3)
	xlims!(ax4,-5,25)
	ylims!(ax4,-5,10)
	f
end
...
manifest_format = "2.0"
project_hash = "e534a33601a70c16be1e19ec83bd1df7edebcd8d"

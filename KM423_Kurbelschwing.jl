...
## Kinematik einer Kurbelschwinge
"""

# ╔═╡ 81b0bd17-1a34-4189-b875-6f36707893c2
md"""
### Einführung
Wir untersuchen eine Kurbelschwinge. Eine Kurbelschwinge ist in Aufgabe 7.11 im Buch *Kinematik und Kinetik* von T. Geike zu finden. Wir untersuchen eine leicht abgewandelte Aufgabenstellung, bei der nicht die Bewegung von Bauteil 2 gesucht ist, sondern von Bauteil 3 (Schwinge). Für Teilnehmende meiner Kurse findet sich die Aufgabe im Foliensatz A1.

Das konkrete Beispiel orientiert sich an der Aufgabe *Kinematik eines Gelenkvierecks* aus dem Buch *Repertorium und Übungsbuch der Technischen Mechanik* von I. Szabó (Springer, 1972). Die Wahl des Schwingenwinkels ist hier anders als im Buch gewählt. Das Durcharbeiten des Beispiels im Buch von Szabó macht deutlich, wie beschwerlich das Lösen solcher Aufgabe ohne Computer war."""

# ╔═╡ def67931-90c3-4479-8868-a044cf660602
md"""
!!! warning "Achtung Einheiten"
    Wir führen im folgenden Programmcode die Einheiten nicht mit. Wir vereinbaren, dass alle Längen in Millimetern, alle Winkel in Radiant und alle Zeiten in Sekunden angegeben sind.

Falls man mit unterschiedlichen Einheiten arbeiten möchte (vielleicht sogar gemischt mit verschiedenen Einheitensystemen), dann ist die Verwendung des Paketes `Unitful` für das Rechnen mit Einheiten sehr zu empfehlen.
"""

# ╔═╡ 17f0fc04-929f-44c0-8d76-6649bb21315a
md"""Bevor wir in die Berechnungen einsteigen, legen wir die geometrischen Parameter fest."""

# ╔═╡ 38bef4e6-58ee-486a-96d3-6daa46d77040
begin
	l0 = 400 # mm
	l1 = 100 # mm
	l2 = 450 # mm
...
# ╔═╡ 75bd2ebd-0b0e-40d6-b6ad-fcbbb4734184
md"""
| Länge | Wert | Einheit |
| :----- | -----: | :-----: |
| Basis/Gestell | $(l0) | mm |
| Kurbel | $(l1) | mm |
| Schwinge | $(l3) | mm |
| Verbindungsteil | $(l2) | mm |
"""

# ╔═╡ 9d66742e-e5bf-4354-953c-72ec33fddc10
md"""
### Vorüberlegungen: Bindungsgleichung
Wir starten mit der Bindungsgleichung
```math
f(\theta,\varphi) = (x_\mathrm{C} - x_\mathrm{B})^2 + (y_\mathrm{C} - y_\mathrm{B})^2 - l_2^2 = 0\;,
```
wobei gilt
```math
x_\mathrm{B} = l_1\cos\theta_1\,,\,y_\mathrm{B} = l_1\sin\theta_1\,,\, x_\mathrm{C} = l_0+l_3\cos\theta_3\,,\, y_\mathrm{C}=l_3\sin\theta_3\,.
```
Die Bindungsgleichung verknüpft den Winkel des Antriebs ``\theta_1`` (Kurbel) mit dem Winkel des Abtriebs ``\theta_3`` (Schwinge). Für einen gegebenen Kurbelwinkel ``\theta_1`` kann mittels dieser Gleichung der Winkel ``\theta_3`` des Abtriebs berechnet werden. Das kann analytisch versucht werden (siehe Buch von Szabó). Das Ergebnis läge dann in Form einer Formel ``\theta_3 = \dots`` vor. 
In vielen Fällen gelingt dies jedoch nicht (nicht mit vertretbarem Aufwand oder gar nicht).
Alternativ kann die Gleichung für beliebige Werte des Kurbelwinkels numerisch gelöst werden. Das Ergebnis ist dann eine Wertetabelle, bei der jedem gewählten Wert des Kurbelwinkels ein Wert für den Abtriebswinkel zugeordnet wird.

Wir nehmen hier ausschließlich den numerischen Weg.
"""


# ╔═╡ dd76fa6b-d0e7-4b98-b7d0-08e802e7e86e
f(θ1,θ3) = (l0 + l3*cos(θ3) - l1*cos(θ1))^2 + (l3*sin(θ3) - l1*sin(θ1))^2 - l2^2;

# ╔═╡ bd6c160a-df73-429a-b9d0-776d130663be
md"""
Wir benötigen später eine Schreibweise für die Bindungsgleichung, die nicht die beiden skalaren Größen ``\theta_1`` und ``\theta_3`` enthält, sondern eine Spaltenmatrix ``\mathbf{q}`` mit zwei Elementen.
Die Spaltenmatrix ``\mathbf{q}`` fasst die beiden Winkel in einer Variablen (in einem mathematischen Objekt) wie folgt zusammen. 
```math
\mathbf{q} = (\theta_1,\, \theta_3)\;.
```
Demnach gilt ``q_1 = \theta_1`` und ``q_2 = \theta_3``.
"""
...
md"""
### Zeitgesetz für den Antrieb
Im Hinblick auf den Antrieb sind verschiedene Vorgehensweisen denkbar. Häufig wird bei Lehrbuchbeispielen der Fall konstanter Winkelgeschwindigkeit untersucht. Dann würde man ``\omega_1 = \dot\theta_1`` fest vorgeben. Eine explizite Zeitabhängigkeit wird dann meist nicht betrachtet und Ergebnisse werden als Funktion des Antriebswinkels dargestellt. Üblicherweise wird man die Winkelgeschwindigkeit des Abtriebs auf ``\omega_1`` beziehen.

Hier gehen wir einen anderen Weg. Wir legen ein Zeitgesetz für den Antriebswinkel vor und bestimmen die entsprechenden Ableitungen mittels automatischer Differentiation. Das Vorgehen funktioniert sowohl bei konstanter Winkelgeschwindigkeit des Antriebs als auch bei anderen Zeitgesetzen.
"""
...
ω1(t) = ForwardDiff.derivative(θ1,t);

# ╔═╡ b2630302-36d2-427f-8d32-f26e9ca95beb
α1(t) = ForwardDiff.derivative(ω1,t);

# ╔═╡ f34dd846-bd25-40d8-843a-6c63d8a297e9
md"""
### Numerisches Lösen der Bindungsgleichung und Berechnung von Winkelgeschwindigkeit und Winkelbeschleunigung der Schwinge"""

# ╔═╡ 17c19c08-6e69-47b4-ae38-7fcf73c14607
N = 300;

# ╔═╡ ee419ece-7a69-4f95-8bd9-7f96c1455fb1
md"""Wir erstellen im Folgenden eine Wertetabelle mit $(N) Punkten. Die Wertetabelle ordnet jedem Kurbelwinkel ``\theta_1`` den zugehörigen Abtriebswinkel ``\theta_3`` sowie die Winkelgeschwindigkeit und die Winkelbeschleunigung des Abtriebs zu.

Hinweis: Für die Bestimmung von Winkelgeschwindigkeit und Winkelbeschleunigung wird die Kettenregel im Kontext impliziter Funktionen benötigt. Dazu kann man im oben genannten Buch *Kinematik und Kinetik* im Abschnitt 2.3 nachlesen."""

# ╔═╡ 52aec6d6-abea-4d86-95bd-69468a88e3a7
begin 
	global θ3_ini = 1.0 #-1.0
	t_tab = LinRange(0.0,8,N);
	θ1_tab = θ1.(t_tab)
	θ3_tab = zeros(N)
	ω3_tab = zeros(N)
	α3_tab = zeros(N)
...
| :----- | -----: | :-----: |
| min. Schwingenwinkel | $(round(rad2deg(minimum(θ3_tab)),digits=2))|°|
| max. Schwingenwinkel | $(round(rad2deg(maximum(θ3_tab)),digits=2))|° |
| min. Winkelgeschwindigkeit der Schwinge | $(round(minimum(ω3_tab),digits=2))| rad/s| 
| max. Winkelgeschwindigkeit der Schwinge | $(round(maximum(ω3_tab),digits=2))| rad/s|
| min. Winkelbeschleunigung der Schwinge | $(round(minimum(α3_tab),digits=2))| rad/s² | 
| max. Winkelbeschleunigung der Schwinge | $(round(maximum(α3_tab),digits=2))| rad/s² 	|
"""

# ╔═╡ 4f51e001-6844-4d34-ba9c-090dbce58b9e
# Hilfsfunktion für das Zeichnen von Kreisen
...
md"""
!!! task "Aufgabe: Start aus einer anderen Konfiguration"
    Für den Anfangswert des Kurbelwinkels ``\theta_1=0`` ist der zugehörige Schwingenwinkel ``\theta_3`` nicht eindeutig. Welche andere Lösung gibt es. Berechnen Sie alle Diagramme für diesen zweiten Fall.
"""
